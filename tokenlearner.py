...
class TokenLearner(nn.Module):
    ...
    def __init__(self, in_channels, num_tokens, use_sum_pooling=False):
        ...
        self.attention_maps = nn.Sequential(
            # 3 layers of conv with gelu activation as suggested in the paper.
    ...
    def forward(self, inputs):
        ...
        selected = self.attention_maps(selected)                 # Shape:  [bs, n_token, h, w] attention_map是一种特殊矩阵的计算方式，凝练出有特点的矩阵数据
        ...
        # 如果在view之前用了transpose，permute等，需要用contiguous()来返回一个contiguous copy，在torch里面，view相当于numpy里的reshape
        selected = selected.permute(0, 2, 1)[..., None]   # Shape: [bs, n_token, h*w, 1].

         # Reshape the input to align it with the output of the conv block.
        feat = inputs
        feat = feat.view(feature_shape[0], feature_shape[1] * feature_shape[2], -1)[:, None, ...]  # Shape: [bs, 1, h*w, c].
        # None表示维度扩充，: 表示切片，...代表所有维度的数据，而:只代表一个维度的数据
...
class TokenLearnerModuleV11(nn.Module):
    ...
    def forward(self, inputs):
        ...
        selected = inputs
        selected = self.norm(selected)
        selected = selected.permute(0, 3, 1, 2)  # Shape:  [bs, c, h, w] permute函数的作用就是对tensor进行转置，将原来放在第0号位置的
        # 参数转置后放到第0号位置，将原来放在第三号位置的参数放到第1号位置上，以此类推，就转置好了
